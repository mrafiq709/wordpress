				...
				if (is_super_admin($user_id) || $message_publish == "publish") {
					if ($user_id != $get_message_user) {
						...
						if ($send_email_message == "on") {
							...
							$send_text = wpqa_send_email(wpqa_options("email_new_message"),$get_message_user,$return,"","","","","","","","","","","","","",$get_post->post_author,$user->ID);
...
function wpqa_process_new_messages() {
	...
	if ($post_type == "send_message") {
		...
		do_action('wpqa_add_message_errors',$errors,$question_add);
