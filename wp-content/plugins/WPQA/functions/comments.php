...
if (!function_exists('wpqa_comment_question_before')) :
	function wpqa_comment_question_before($commentdata) {
		...
		if (!is_admin() && ($get_post_type_comment == "question" || $get_post_type_comment == "post" || $get_post_type_comment == "page")) {
			...
			if ($terms_active_comment == "on" && $_POST['agree_terms'] != "on") {
		...
		$commentdata["comment_content"] = ($comment_editor == "on"?wpqa_kses_stip($commentdata['comment_content'],"","yes"):wpqa_kses_stip_wpautop($commentdata['comment_content']));
		return $commentdata;
	}
endif;
/* Comment question */
	...
	function wpqa_comment_question($comment_id) {
		...
		if ($get_comment->comment_approved != 1) {
			...
			if ($send_email_draft_comments == "on") {
				$send_text = wpqa_send_email(wpqa_options("email_draft_comments"),"",$post_id,$comment_id);
...
if (!function_exists('wpqa_notifications_add_answer')) :
	function wpqa_notifications_add_answer($comment,$get_post) {
		...
		if ($remember_answer == "on" && (($post_author != $comment_user_id) || ($post_author == 0 && $comment_user_id == 0))) {
			if ($post_author > 0) {
			...
			$send_text = wpqa_send_email(wpqa_options("email_notified_answer"),"",$post_id,$comment_id,"","","","","","","","","","","","",($comment_user_id > 0?$comment_user_id:$comment),(isset($anonymously_user) && $anonymously_user > 0?$anonymously_user:$post_author));
			$last_message_email = wpqa_email_code($send_text);
			...
			if ($unsubscribe_mails != "on" || ($answer_on_your_question == "on" || $answer_on_your_question == "")) {
				wpqa_sendEmail($email_template,get_bloginfo('name'),$the_mail,$the_author,$email_title,$last_message_email);
			}
		}
			...
			foreach ($following_questions as $user) {
				if ($user_id_question != $user) {
					...
					if ($user > 0 && $comment_user_id != $user) {
						$send_text = wpqa_send_email(wpqa_options("email_follow_question"),"",$post_id,$comment_id,"","","","","","","","","","","","",($comment_user_id > 0?$comment_user_id:$comment),$user);
						$last_message_email = wpqa_email_code($send_text);
						...
						if ($unsubscribe_mails != "on" || ($answer_question_follow == "on" || $answer_question_follow == "")) {
							wpqa_sendEmail($email_template,get_bloginfo('name'),esc_html($author_user_email),esc_html($author_display_name),$email_title,$last_message_email);
						}
					}
				}
			}
...
/* Comment login */
add_filter("discy_filter_comment_form","wpqa_filter_comment_form",1,2);
...
add_action("discy_action_after_share_comment","wpqa_after_share_answer_replies",1,2);
...
if (!function_exists('wpqa_new_answers_page_link')) :
	function wpqa_new_answers_page_link($status_links) {
		$status_links['comments'] = '<a href="edit-comments.php?comment_status=all&amp;comments=1"'.(isset($_GET['comments'])?' class="current"':'').'>'.esc_html__('Comments','wpqa').' ('.wpqa_all_comments_of_post_type("post").')</a>';
		$status_links['answers'] = '<a href="edit-comments.php?comment_status=all&amp;answers=1"'.(isset($_GET['answers'])?' class="current"':'').'>'.esc_html__('Answers','wpqa').' ('.wpqa_all_comments_of_post_type("question").')</a>';
		return $status_links;
	}
endif;?>