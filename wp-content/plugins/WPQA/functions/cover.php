...
function wpqa_cover_image () {
	...
	if ($cover_image == "on" && wpqa_is_user_profile() && !wpqa_is_user_edit_profile()) {
		...
		echo "<div class='wpqa-profile-cover".($cover_fixed == "fixed"?" wpqa-cover-fixed":"").($profile_credential != ""?" cover-with-credential":"")."'>
			...
			<div class='wpqa-cover-inner".($cover_fixed == "fixed"?"":" discy-container")."'>
				<div class='wpqa-cover-content'>
					...
					<div class='wpqa-cover-right'>";
						...
						if ($owner || ($active_moderators == "on" && isset($if_user_id->roles[0]) && $if_user_id->roles[0] != "administrator" && $pending_questions == true && ($is_super_admin || (isset($moderators_permissions['ban']) && $moderators_permissions['ban'] == "ban")))) {
							echo "<div class='question-list-details'>
								...
								<ul class='question-link-list'>";
									...
									if ($active_moderators == "on" && isset($if_user_id->roles[0]) && $if_user_id->roles[0] != "administrator" && $pending_questions == true && ($is_super_admin || (isset($moderators_permissions['ban']) && $moderators_permissions['ban'] == "ban"))) {
										echo "<li><span class='small_loader loader_2'></span><a class='".(isset($if_user_id->caps["ban_group"]) && $if_user_id->caps["ban_group"] == 1?"unban-user":"ban-user")."' data-nonce='".wp_create_nonce("ban_nonce")."' href='#' data-id='".$user_id."'><i class='".(isset($if_user_id->caps["ban_group"]) && $if_user_id->caps["ban_group"] == 1?"icon-back":"icon-cancel-circled")."'></i><span>".(isset($if_user_id->caps["ban_group"]) && $if_user_id->caps["ban_group"] == 1?esc_html__("Unban user","wpqa"):esc_html__("Ban user","wpqa"))."</span></a></li>";
									}
								echo "</ul>
							</div>";
						}
