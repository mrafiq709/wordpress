...
if (!function_exists('wpqa_comment_reply_link')) :
	function wpqa_comment_reply_link($link,$args,$comment) {
		...
		if ($comment_editor == "on") {
			$link = '<li><a rel="nofollow" class="comment-reply-link wpqa-reply-link" href="'.get_the_permalink($comment->comment_post_ID).'#respond" data-id="'.$comment->comment_ID.'" aria-label="'.esc_attr(sprintf($args['reply_to_text'], $comment->comment_author)).'"><i class="icon-reply"></i>'.esc_html__("Reply","wpqa").'</a></li>';
		}
		return $link;
	}
endif;?>