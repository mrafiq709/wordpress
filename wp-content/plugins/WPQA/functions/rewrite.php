	...
	function wpqa_redirect_pages() {
		if (is_author()) {
			...
			if (isset($user_login) && !is_object($user_login)) {
				$user_login = get_user_by('login',urldecode(esc_attr(get_query_var('author_name'))));
			}
			if (isset($user_login) && !is_object($user_login)) {
				$user_login = get_user_by('slug',urldecode(esc_attr(get_query_var('author_name'))));
			}
		...
		if (wpqa_is_search()) {
			if (get_option('permalink_structure') && (!empty($_GET['search']) || !empty($_GET['s']) || !empty($_GET['search_type']) || !empty($_GET['page']) || !empty($_GET['paged']))) {
...
if (!function_exists('wpqa_rewrite_rules')) :
	function wpqa_rewrite_rules() {
		...
		$search_root_rule = $buy_points_root_rule = $login_root_rule = $signup_root_rule = $lost_password_root_rule = $add_category_root_rule = $add_questions_root_rule = $edit_questions_root_rule = $add_posts_root_rule = $edit_posts_root_rule = $edit_comments_root_rule  = '/?$';
		$search_type_rule         = $main_rule.'([^/]+)/?$';
		$search_page_rule         = $main_rule.'([^/]+)/([0-9]{1,})/?$';
		
		// Search All
		add_rewrite_rule( $search_slug . $search_page_rule,                 'index.php?' . $search_id . '&search=$matches[1]&' . $search_type .  '=$matches[2]&' . $page_id . '=$matches[3]', $priority );
		add_rewrite_rule( $search_slug . $search_type_rule,                 'index.php?' . $search_id . '&search=$matches[1]&' . $search_type .  '=$matches[2]',                              $priority );
		add_rewrite_rule( $search_slug . $search_root_rule,                 'index.php?' . $search_id . '&search=$matches[1]',                                                                $priority );
...
if (!function_exists('wpqa_template_include')) :
	function wpqa_template_include ($new_template) {
		...
		if (wpqa_is_user_profile() || wpqa_is_search() || wpqa_is_buy_points() || wpqa_is_login() || wpqa_is_signup() || wpqa_is_lost_password() || wpqa_is_add_category() || wpqa_is_add_questions() || wpqa_is_edit_questions() || wpqa_is_add_posts() || wpqa_is_edit_posts() || wpqa_is_edit_comments() || $wpqa_template_filter == true) {
			global $wp_query;
...
if (!function_exists('wpqa_parse_query')) :
	function wpqa_parse_query($posts_query) {
		...
		if ( !empty( $is_user ) || isset($posts_query->query_vars[apply_filters('wpqa_search_id','search')]) || isset($posts_query->query_vars[apply_filters('wpqa_buy_points','wpqa_buy_points')]) || isset($posts_query->query_vars[apply_filters('wpqa_login','wpqa_login')]) || isset($posts_query->query_vars[apply_filters('wpqa_signup','wpqa_signup')]) || isset($posts_query->query_vars[apply_filters('wpqa_lost_password','wpqa_lost_password')]) || isset($posts_query->query_vars[apply_filters('wpqa_add_categories','add_category')]) || isset($posts_query->query_vars[apply_filters('wpqa_add_questions','add_question')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_questions','edit_question')]) || isset($posts_query->query_vars[apply_filters('wpqa_add_posts','wpqa_add_post')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_posts','edit_post')]) || isset($posts_query->query_vars[apply_filters('wpqa_edit_comments','edit_comment')]) ) {
		...
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_search_id','search')]) ) {
			$search_terms = wpqa_search_terms();
			if ( !empty( $search_terms ) )
				$posts_query->wpqa_search_terms = $search_terms;
			$posts_query->wpqa_is_search = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_buy_points','wpqa_buy_points')]) ) {
			$posts_query->wpqa_is_buy_points = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_login','wpqa_login')]) ) {
			$posts_query->wpqa_is_login = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_signup','wpqa_signup')]) ) {
			$posts_query->wpqa_is_signup = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_lost_password','wpqa_lost_password')]) ) {
			$posts_query->wpqa_is_lost_password = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_add_categories','add_category')]) ) {
			$posts_query->wpqa_is_add_category = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_add_questions','add_question')]) ) {
		...
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_edit_questions','edit_question')]) ) {
			$posts_query->wpqa_is_edit_questions = true;
			if ( !empty( $the_question->ID ) ) {
				$posts_query->set('edit_question',$the_question->ID);
			}
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_add_posts','wpqa_add_post')]) ) {
			$posts_query->wpqa_is_add_posts = true;
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_edit_posts','edit_post')]) ) {
			$posts_query->wpqa_is_edit_posts = true;
			if ( !empty( $the_post->ID ) ) {
				$posts_query->set('edit_post',$the_post->ID);
			}
		}elseif ( isset($posts_query->query_vars[apply_filters('wpqa_edit_comments','edit_comment')]) ) {
...
if (!function_exists('wpqa_search_link')) :
	function wpqa_search_link($query = "",$type = "",$page = "") {
		...
		if (get_option('permalink_structure')) {
			$search_slug = wpqa_options("search_slug");
			$url = home_url('/'.$search_slug.'/'.($query != ""?$query.'/':'').($query != "" && $type != ""?$type.'/':'').($paged != ""?esc_attr($paged).'/':'').($user_filter != ''?'?user_filter='.$user_filter:''));
		}else {
...
/* Get edit url question - post - comment */
...
function wpqa_get_logout() {
	...
	$url = esc_url_raw(wp_logout_url(wp_unslash($redirect_to)));
	...
	function wpqa_author_link($link,$user_id,$user_nicename) {
		...
		if (get_option('permalink_structure')) {
			...
			if ( empty( $user_nicename ) || $profile_type == 'nicename' ) {
				$user_nicename = esc_attr(urlencode(urldecode(trim(wpqa_get_user_nicename($user_id)))));
			}else {
				$user_nicename = esc_attr(urlencode(urldecode(trim(wpqa_get_user_login($user_id)))));
			}
...
/* Load the title */
add_filter('document_title_parts', 'wpqa_the_title');
function wpqa_the_title($title) {
	if (wpqa_is_search()) {
		$title['title'] = esc_html__('Search','wpqa');
	}else if (wpqa_is_buy_points()) {
		$title['title'] = esc_html__('Buy points','wpqa');
	}else if (wpqa_is_login()) {
		$title['title'] = esc_html__('Login','wpqa');
	}else if (wpqa_is_signup()) {
		$title['title'] = esc_html__('Signup','wpqa');
	}else if (wpqa_is_lost_password()) {
		$title['title'] = esc_html__('Lost password','wpqa');
	}else if (wpqa_is_add_category()) {
		$title['title'] = esc_html__('Add category','wpqa');
	}else if (wpqa_is_add_questions()) {
		$title['title'] = esc_html__('Ask question','wpqa');
	}else if (wpqa_is_edit_questions()) {
		$title['title'] = esc_html__('Edit question','wpqa');
	}else if (wpqa_is_edit_tags()) {
		$title['title'] = esc_html__('Edit tags','wpqa');
	}else if (wpqa_is_add_posts()) {
		$title['title'] = esc_html__('Add post','wpqa');
	}else if (wpqa_is_edit_posts()) {
		$title['title'] = esc_html__('Edit post','wpqa');
	}else if (wpqa_is_edit_comments()) {
		$title['title'] = esc_html__('Edit comment','wpqa');
	}else if (wpqa_is_user_profile()) {
		if (wpqa_profile_title()) {
			$title['title'] = wpqa_profile_title();
		}else {
			$wpqa_user_id = (int)get_query_var(apply_filters('wpqa_user_id','wpqa_user_id'));
			$profile_type = wpqa_options("profile_type");
			if ($profile_type == "login") {
				$user_name = urldecode(wpqa_get_user_login($wpqa_user_id));
			}else {
				$user_name = urldecode(wpqa_get_user_nicename($wpqa_user_id));
			}
