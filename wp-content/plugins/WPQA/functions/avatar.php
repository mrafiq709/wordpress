...
if (!function_exists('wpqa_get_user_avatar_image')) :
	function wpqa_get_user_avatar_image ($your_avatar,$size,$user_name,$user_meta_avatar,$user_id) {
		...
		$avatar = "<img class='avatar avatar-".$size." photo' alt='".esc_attr($user_name)."' title='".esc_attr($user_name)."' width='".$size."' height='".$size."' src='".$image."'>";
		return $avatar;
	}
endif;
...
if (!function_exists('wpqa_get_user_avatar_link')) :
	function wpqa_get_user_avatar_link ($args = array()) {
		...
		if ((($your_avatar && !is_array($your_avatar)) || (is_array($your_avatar) && isset($your_avatar["id"]) && $your_avatar["id"] != 0)) && $user_id > 0) {
