...
/* Related questions */
add_action( 'widgets_init', 'widget_related_widget' );
function widget_related_widget() {
...
class Widget_Related extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		if (is_singular("question")) {
			...
			$args = array_merge($related_query_,array('post_type' => 'question','post__not_in' => array($post->ID),'posts_per_page'=> $related_number,'cache_results' => false,'no_found_rows' => true,"meta_query" => array((esc_html($get_question_user_id) != ""?array("type" => "numeric","key" => "user_id","value" => (int)$get_question_user_id,"compare" => "="):array("key" => "user_id","compare" => "NOT EXISTS")))));
