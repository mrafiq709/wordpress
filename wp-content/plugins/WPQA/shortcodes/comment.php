...
function wpqa_edit_comment_attr() {
	...
	if (isset($comment_id) && $comment_id != 0 && $get_post) {
		if (current_user_can('edit_comment',$comment_id) || is_super_admin($user_id) || ($can_edit_comment == "on" && $get_comment->comment_approved == 1 && $get_comment->user_id == $user_id && $get_comment->user_id > 0 && $user_id > 0 && ($can_edit_comment_after == 0 || $time_end <= $can_edit_comment_after))) {
		...
		$out .= '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("Sorry no comment has you select or not found.","wpqa").'</p></div>';
	}
