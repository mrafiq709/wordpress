...
if (!function_exists('wpqa_select_categories')) :
	function wpqa_select_categories ($rand,$select,$attr = array(),$post_id = '',$taxonomy = '',$name = 'category',$show_option_none = '') {
		...
		if ($category_single_multi == "ajax" && $taxonomy != "category") {
			...
			$exclude = apply_filters('wpqa_exclude_question_category',array());
			return '<span class="styled-select">'.wp_dropdown_categories(array_merge($exclude,array("orderby" => "name","echo" => "0","show_option_none" => $show_option_none,'taxonomy' => $taxonomy, 'hide_empty' => 0,'depth' => 0,'id' => ($taxonomy == "question-category"?"question-category":"post-category").'-'.$rand,'name' => $name,'hierarchical' => true,'selected' => $select))).'</span>';
		}
	}
endif;
/* Child cats */
...
function wpqa_add_edit_question($type,$popup = false,$user = false) {
	...
	'<form class="form-post wpqa_form" method="post" enctype="multipart/form-data">'.(isset($_POST["form_type"]) && $_POST["form_type"] == $type."_question"?apply_filters('wpqa_'.($user == "user"?"user":$type).'_question',($user == "user"?"user":$type)):"").'
				...
				foreach ($question_sort as $sort_key => $sort_value) {
					...
					if ($sort_key == "title_question" && ((isset($question_sort["title_question"]["value"]) && $question_sort["title_question"]["value"] == "title_question") || (isset($question_sort["comment_question"]["value"]) && $question_sort["comment_question"]["value"] != "comment_question"))) {
						...
						$out .= ($suggest_questions == "on"?'<div class="the-title-div">':'<p>').
							...
							<input name="title" id="question-title-'.$rand.'" class="the-title'.($suggest_questions == "on"?" suggest-questions live-search live-search-icon":"").'" type="text" value="'.($type == "add" && isset($question_add['title'])?wpqa_kses_stip($question_add['title']):($type == "edit"?(isset($question_edit['title'])?wpqa_kses_stip($question_edit['title']):wpqa_kses_stip($get_post_q->post_title)):"")).'">
							<i class="icon-chat"></i>
	...
	if (isset($_POST["form_type"]) && $_POST["form_type"] == "add_question" && empty($_POST["user_id"])) :
		...
		if (is_wp_error($return)) :
   			...
   			if ($get_post->post_type == "question") {
	   			...
	   			$question_status = wpqa_get_question_status($return,$get_current_user_id);
...
function wpqa_user_question($type) {
	if (isset($_POST["form_type"]) && $_POST["form_type"] == "add_question" && isset($_POST["user_id"]) && $_POST["user_id"] != "") :
		$return = wpqa_process_new_questions("user");
		if (is_wp_error($return)) :
   			return '<div class="wpqa_error">'.$return->get_error_message().'</div>';
   		else :
   			$get_post = get_post($return);
   			if ($get_post->post_type == "question") {
   				$user_id = get_current_user_id();
   				$question_status = wpqa_get_question_status($return,$user_id);
   	...
   	elseif (isset($_POST["form_type"]) && $_POST["form_type"] == "edit_question") :
		$return = wpqa_process_edit_questions("user");
		if (is_wp_error($return)) :
			return '<div class="wpqa_error">'.$return->get_error_message().'</div>';
		else :
...
function wpqa_process_new_questions($user = "") {
	global $question_add;
	set_time_limit(0);
	$errors = new WP_Error();
	$question_add = array();
	...
	if ($form_type == "add_question") {
		...
		if (!isset($_POST['wpqa_add'.$user.'_question_nonce']) || !wp_verify_nonce($_POST['wpqa_add'.$user.'_question_nonce'],'wpqa_add'.$user.'_question_nonce')) {
		...
		if ($title_question === "on") {
			...
			$question_title = wpqa_excerpt_any($title_excerpt,sanitize_text_field($question_add['comment']),$title_excerpt_type);
		}
		...
		if ((empty($question_add['user_id']) || (isset($question_add['user_id']) && $question_add['user_id'] == "")) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question" && isset($question_add['category']) && $question_add['category']) {
			...
			if ($category_single_multi == "ajax_2") {
				...
				if ($term !== 0 && $term !== null) {
					...
					wpqa_add_cateory_request($question_add['category'],$question_id);
				}
			}else {
...
function wpqa_question($atts, $content = null) {
	...
	if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_get_current_user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
		$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have a permission to ask a question","wpqa").(!is_user_logged_in()?", ".esc_html__("You must login to ask question","wpqa"):'').'.</p></div>';
		if (!is_user_logged_in()) {
		...
		if (!is_user_logged_in() && $pay_ask == "on") {
			...
			if (isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && $payment_group[$user_login_group] != $user_login_group) {
				$out .= wpqa_get_payment_coupons($user_get_current_user_id,(isset($a["popup"]) && $a["popup"] == "popup"?"popup":""),0,0,"ask_question",0,0,"",esc_html__("Please make a payment to allow to be able to ask a question %s.","wpqa"));
			}else {
				$question_points_active = wpqa_options("question_points_active");
	...
	if (!is_user_logged_in()) {
		...
		if ($edit_question == "on" || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
			...
			if (isset($get_post) && $get_post != 0 && $get_question && $get_question->post_type == "question") {
				if ($get_question->post_author != 0 || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
					if (($get_question->post_author == $user_id && $user_id != 0 && $get_question->post_status == "publish") || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
...
function wpqa_edit_question() {
	if (isset($_POST["form_type"]) && $_POST["form_type"] == "edit_question") :
		...
		if (is_wp_error($return)) :
   			...
   			$user_id = get_current_user_id();
   			$moderators_permissions = wpqa_user_moderator($user_id);
			if ($question_approved == "on" || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit") || is_super_admin(get_current_user_id())) {
				wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("Has been edited successfully.","wpqa").'</p></div>','wpqa_session');
...
function wpqa_process_edit_questions($user = "") {
	...
	if ($form_type == "edit_question") {
		...
		$fields = array(
			'title','comment','category','question_poll','question_image_poll','remember_answer','question_tags','video_type','video_id','video_description','sticky','ask','private_question','anonymously_question','featured_image','publish_question'
		);
		...
		if (!isset($_POST['wpqa_edit'.$user.'_question_nonce']) || !wp_verify_nonce($_POST['wpqa_edit'.$user.'_question_nonce'],'wpqa_edit'.$user.'_question_nonce')) {
			$errors->add('required-field','<strong>'.esc_html__("Error","wpqa").' :&nbsp;</strong> '.esc_html__("There is an error, Please try again.","wpqa"));
		}
		...
		if (isset($get_question) && $get_question != 0 && $get_post_q && $get_post_q->post_type == "question") {
			$moderators_permissions = wpqa_user_moderator($user_id);
			if (($get_post_q->post_author == $user_id && $user_id != 0 && $get_post_q->post_status == "publish") || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
				// Yes, you can edit.
			}else {
			...
			if (isset($question_edit['category']) && $question_edit['category']) {
				...
				if ($category_single_multi == "ajax_2") {
					...
					if ($term !== 0 && $term !== null) {
						...
						wpqa_add_cateory_request($question_edit['category'],$question_id);
					}
				}else {
...
function wpqa_add_edit_question_errors($errors,$posted,$type,$question_sort,$get_question_user_id,$comment_question,$title_question) {
	...
	if ($type == "edit") {
		if (empty($get_question_user_id) && isset($question_sort["categories_question"]["value"]) && $question_sort["categories_question"]["value"] == "categories_question" && (empty($posted['category']) || $posted['category'] == '-1')) {
			$errors->add('required-field','<strong>'.esc_html__("Error","wpqa").' :&nbsp;</strong> '.esc_html__("There are required fields (category).","wpqa"));
		}
	}
